...
public class Module_Editor extends DefaultNodeEditor {
  ...
  private EditorCell createCollection_iuuvkg_a(EditorContext editorContext, SNode node) {
    ...
    editorCell.addEditorCell(this.createRefNode_iuuvkg_d0(editorContext, node));
    return editorCell;
  }
  ...
  private EditorCell createRefNode_iuuvkg_d0(EditorContext editorContext, SNode node) {
    CellProviderWithRole provider = new RefNodeCellProvider(node, editorContext);
    provider.setRole("entityResource");
    provider.setNoTargetText("<no entityResource>");
    EditorCell editorCell;
    editorCell = provider.createEditorCell(editorContext);
    if (editorCell.getRole() == null) {
      editorCell.setRole("entityResource");
    }
    editorCell.setSubstituteInfo(provider.createDefaultSubstituteInfo());
    SNode attributeConcept = provider.getRoleAttribute();
    Class attributeKind = provider.getRoleAttributeClass();
